...
exports.verifyEmail = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError('make sure that you signed up', 400));
  }
...
exports.confirmEmail = catchAsync(async (req, res, next) => {
  ...
  const confirmtoken = user.confirmEmailToken;
...
exports.contactUs = catchAsync(async (req, res, next) => {
  ...
  const URL = `#`;
  // Send the email to the support address
  try {
    await new Email({ name, email, message }, URL, subject, true).sendContactUs(
      message,
    );
    res.status(200).json({
      status: 'success',
    ...
    return next(
      new AppError('error sending your message. Try again later!', 500),
    );
  }
});
